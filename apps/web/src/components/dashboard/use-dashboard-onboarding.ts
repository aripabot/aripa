...
import type {
  DashboardOnboardingStep as Step,
...
export function useDashboardOnboarding({
  ...
  function copyGeneratedKeyForStep(): void {
    void copyGeneratedKey();
  }

  function generateKeyPairForStep(): void {
    void generateKeyPair();
  }

  const stepContentProps: DashboardOnboardingStepContentProps = {
    allowlistInput,
    config,
    ...
    updateModel,
    updateSource,
    onCopyGeneratedKey: copyGeneratedKeyForStep,
    onGenerateKeyPair: generateKeyPairForStep,
  };

  return {
    activeProgressIndex: stepIndex(step),
